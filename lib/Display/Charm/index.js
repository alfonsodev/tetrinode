...
Charm.prototype.printDebug = function(x, y, tetro, matrix) {
  var m = tetro.getMatrix();
  var i = (m[0].indexOf(1) >= 0)? m[0].indexOf(1) >= 0 : 0; 
  charm.position(x, y + 1);
  // charm.delete('char', 5);
  this.switchColor(0);
  charm.write( tetro.posX.toString() + ', ' + tetro.posY.toString());
  charm.down();
  var a = tetro.posX + i - 1;
  charm.write(a.toString());
  charm.down();
  charm.write(i.toString());
};
